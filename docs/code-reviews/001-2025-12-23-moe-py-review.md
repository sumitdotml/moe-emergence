...
Independent code review of `moe_emergence/moe.py` conducted by two models (Claude Opus 4.5 and GPT-5.2) against the V3 design specification (`project-design/MOE-PROJECT-DESIGN-V3.md`). Both reviewers converged on the same findings.
...
All fixes applied in commit `eea9294`. See `002-2025-12-23-moe-py-fix.md` for details.
...
- [x] Verify entropy logging uses clean probs (verified in `moe_emergence/verify_gpt2_integration.py`)
- [ ] Add per-layer entropy tracking in training loop

### Optional Improvements

- [x] ~~Support 2D input in Router~~ (won't fix; 3D-only contract, see `docs/decisions/004-router-3d-only-input.md`)
...
- GPT-5.2 Review: `docs/models-debate/MODELS-DEBATE.md`
