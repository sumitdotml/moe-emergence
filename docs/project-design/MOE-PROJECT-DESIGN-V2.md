...
**Version 2 Updates**: This revision incorporates critical bug fixes identified in the final review round, including the top-1 routing gradient flow issue, proper expert initialization via deepcopy, correct HuggingFace integration via drop-in replacement, sequence packing instead of padding, and the complete efficient batched dispatch implementation.
...
**The Fix**: For top-1 routing, keep the original softmax probability as the gate scalar (don't renormalize). For top-k > 1, renormalization over selected experts is fine because relative weights still carry gradient information.
...
class Router(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # CRITICAL FIX: Handle top-1 vs top-k differently for gradient flow
        if self.top_k == 1:
            # For top-1: Keep original probability, don't renormalize to 1.0
            # This preserves gradient flow from LM loss to router
            weights = top_k_probs  # Shape: [batch * seq_len, 1]
        else:
        ...
        return {
            ...
            'router_probs': clean_probs,     # [batch * seq_len, num_experts] - for load balancing
...
**Verification**: For top-1, verify that `weights` values are less than 1.0 (actual probabilities, not renormalized). Verify that `torch.autograd.grad(output.sum(), router.gate.weight)` is non-None.
...
class MoEWrapper(nn.Module):
    ...
    def __init__(self, original_mlp, num_experts=8, top_k=1, noise_std=0.1):
        super().__init__()
        
        self.num_experts = num_experts
        self.top_k = top_k
        
        # Infer hidden size from original MLP
        # GPT2MLP has c_fc (hidden -> intermediate) and c_proj (intermediate -> hidden)
        hidden_size = original_mlp.c_fc.weight.shape[1]  # Conv1D weight is [out, in]
        ...
        for code in tqdm(code_samples, desc="Analyzing token types"):
            ...
            for bpe_idx, (start_char, end_char) in enumerate(offset_mapping):
                ...
                for py_tok in python_tokens:
                    py_start = py_tok.start[1] if py_tok.start[0] == 1 else 0  # Simplified
                    py_end = py_tok.end[1] if py_tok.end[0] == 1 else len(code)
                    
                    # Check overlap (simplified - real implementation needs line handling)
                ...
                for aux in aux_outputs:
                    layer_idx = aux['layer_idx']
                    expert_idx = aux['indices'][0, bpe_idx, 0].item()  # top-1
...
A: The selection (which expert) is non-differentiable, but the weight (how much to scale the expert output) is differentiable. By keeping the original softmax probability rather than renormalizing to 1.0, we ensure the LM loss gradient flows through the weight to the router parameters. The router learns whether its probability assignments were good, even if it can't learn about counterfactual expert choices.
...
*Document Version 2.0 - Incorporates critical bug fixes from final review round*
